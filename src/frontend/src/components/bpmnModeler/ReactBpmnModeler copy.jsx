...
const ReactBpmnModeler = ({
  ...
  */
  ...
  const handleDelete = () => {
    ...
    try {
      modeling.setColor(baseTask, { fill: "#f1948a" }); 
    } catch (err) {
      console.error(
        "Could not execute delete pattern on task: ",
        baseTask,
        err
      );
    }

    baseTask.businessObject.customType = "delete";
    baseTask.businessObject.customBaseTask =
      selectedElement?.businessObject?.name || "";

    updateChangePattern({
      bpmnElementId: baseTask.id,
      type: "delete",
  ...
  // Replace: Creates a new task and connects it in both directions with the selected task.
  const handleReplace = () => {
    if (!selectedElement) {
      setSnackbarErrorMessage("Please select a task first");
      setSnackbarErrorOpen(true);
